...
logging.basicConfig(filename = "IPCAM/Caplog.log",
                    filemode = "w",
                    format = Log_Format,
                    level = logging.INFO)
...
class LoadStreams:  # multiple IP or RTSP cameras
    ...
    def update(self, index, cap, url):
        # Read next stream frame in a daemon threadSS
        n = 0
        while(True):
            while cap.isOpened():
                n += 1
                # _, self.imgs[index] = cap.read()
                if cap.grab():    #下一幀   分開解碼節省資源只在每4幀解碼一次
                    if n == 4:  # read every 4th frame
                        success, im = cap.retrieve()    #解碼
                        self.imgs[index] = im if success else self.imgs[index] * 0
                        n = 0
                else:
                    print(f"{url} 無法讀取攝影機畫面")
                    logger.error(f"{url} Disconnected")
                    self.error += 1
                    cap.release()
                time.sleep(1 / self.fps)  # wait time

            cap = cv2.VideoCapture(url)   #嘗試重啟攝影機
            if cap.isOpened():  #查看是否重啟成功
                self.fps = cap.get(cv2.CAP_PROP_FPS) % 100
                print(f'Success to open {url}')
                logger.info(f"{url} Success connected")
                self.error -= 1
            else: print(f'Failed to open {url}')
            time.sleep(5)  #等待5秒嘗試重啟攝影機
    ...
    def __next__(self):
        self.count += 1
        img0 = self.imgs.copy()
        if self.error == self.index:  #如果錯誤攝影機等於停車場所有攝影機就退出
            exit()
        ...
        for x in img0[1:]:
            #img_bind = cv2.vconcat([x, img_bind])
            self.img_size += self.size   #增加大小
            img0[0] = np.concatenate((x,img0[0]), axis=1)

        # Letterbox
        #img = [letterbox(x, self.img_size, auto=self.rect, stride=self.stride)[0] for x in img0]
        img = [letterbox(img0[0], self.img_size, auto=self.rect, stride=self.stride)[0]]
        # Stack
        img = np.stack(img, 0)
